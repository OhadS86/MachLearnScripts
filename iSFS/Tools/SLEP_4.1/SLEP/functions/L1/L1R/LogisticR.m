...
x = [0.0, -0.40380071626799474, 1.621060683555652, 0.0, -0.1113209842878034, 0.0, 0.0, 0.0, -0.5312360868140361, 2.5947084053801373]'; c = -0.2620881168141119;
aa=- y.*(A * x+ c);
bb=max(aa,0);
fun_x= weight' * ( log( exp(-bb) +  exp(aa-bb) ) + bb )

fprintf ('obj = %.9f, reg = %.9f\n', fun_x + lambda* sum(abs(x)), lambda* sum(abs(x)))
weight
sm = log( exp(-bb) +  exp(aa-bb) ) + bb
