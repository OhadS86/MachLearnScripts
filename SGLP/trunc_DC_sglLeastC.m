function [ x, fval ] = trunc_DC_sglLeastC( A, y, s1, s2, opts )
%TRUNC_DC_SGLLEASTC Summary of this function goes here
%     \min_{x}      \| Ax - y \|_2 + 1/2 rsL2 * ||x||_2^2
...
if (s1 < 0 || s2 < 0)
    error('\n s1 and s2 should be nonnegative!\n');
end

if ~isfield(opts,'ind')
    error('\n In trunc_DC_sglLeastC, .ind should be specified');
end
	
if ~isfield(opts,'supp1')
    error('\n In trunc_DC_sglLeastC, .supp1 should be specified');
end
	
if ~isfield(opts,'suppG')
    error('\n In trunc_DC_sglLeastC, .suppG should be specified');
else
...
if (opts.nFlag~=0)
    ...
    ind_zero= (abs(nu)<= 1e-10);    nu(ind_zero)=1;
...
if (opts.lFlag==0)
    ...
    for iterStep=1:opts.maxIter
        if mod(iterStep, 500) == 0
            fprintf('\t Trunc DC sglLeastC Iteration: %d\n', iterStep);
        end
%         tic;
        % --------------------------- step 1 ---------------------------
        ...
        while (1)
            ...
            % projection
            x = trunc_sglp(v, s1, s2, ind, supp1, suppG);
% 			x = trunc_sglp_cvx( v, s1, s2, ind, supp1, suppG );
% 			fprintf('Difference between cvx and sglp = %.5f\n', norm(x-x_cvx));
...
% 		fprintf('iteration %d, fval = %.5f\n', iterStep, funVal(iterStep));
    end 
end % end of .lFlag=0
...
if (opts.lFlag==1)
    ...
    for iterStep = 1 : opts.maxIter
        ...
        while (1)
            ...
            % projection
            xnew = trunc_sglp(v, s1, s2, ind, supp1, suppG);
%             lambda0=lambda;
