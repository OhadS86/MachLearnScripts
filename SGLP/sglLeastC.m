function [ x, fval ] = sglLeastC( A, y, s1, s2, opts )
%SGLLEASTC Summary of this function goes here
%   \min_{x}      \| Ax - y \|_2 + 1/2 rsL2 * ||x||_2^2
%    subject to    \| x \|_1 <= s1
%                  \sum \|x_{G_i}\|_2 <= s2
%
% The group information is contained in the array ind
...
if ~isfield(opts,'ind')
    error('\n In sglLeastC, .ind should be specified');
end
...
%         tic;
        ...
        while (1)
            ...
            % projection
%              tmpTimer = cputime;
%             x = sglpMatlab(v, s1, s2, ind);
%              fprintf('Projection takes %d\n', cputime - tmpTimer);
            x = sglp(v, s1, s2, ind);
